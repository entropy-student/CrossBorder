import type {
  ...
  PayPalRetrieveRefundResponse,
  PayPalRetrieveCaptureResponse,
...
export class PayPalHttpTransport implements PayPalTransport {
  ...
  constructor(options: PayPalHttpTransportOptions) {
    if (options.environment !== "sandbox") throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "PayPal live transport is disabled until a separate live-readiness review.")
  ...
  private async accessToken(): Promise<string> {
    ...
    if (!token || !Number.isFinite(expiresIn) || expiresIn <= 0) throw new MedusaError(MedusaError.Types.INVALID_DATA, "PayPal OAuth response did not contain a valid temporary access token.")
  ...
  private async request(pathname: string, init: RequestInit, authenticate = true): Promise<Record<string, unknown>> {
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt += 1) {
      ...
      try {
        ...
        lastError = error
        if (error instanceof PayPalHttpError && error.status === 401 && authenticate && !refreshedAfterUnauthorized) {
    ...
    throw lastError instanceof Error ? lastError : new MedusaError(MedusaError.Types.UNEXPECTED_STATE, "PayPal request failed.")
  }
  ...
  async createOrder(input: Parameters<PayPalTransport["createOrder"]>[0]): Promise<PayPalCreateOrderResponse> {
    ...
    if (!id || !stringValue(body.status)) throw new MedusaError(MedusaError.Types.INVALID_DATA, "PayPal create-order response was malformed.")
  ...
  async authorizeOrder(input: Parameters<PayPalTransport["authorizeOrder"]>[0]): Promise<PayPalAuthorizeOrderResponse> {
    ...
    if (!orderId || !stringValue(body.status) || !evidence.authorization_id) throw new MedusaError(MedusaError.Types.INVALID_DATA, "PayPal authorize response was malformed.")
  ...
  async captureAuthorization(input: Parameters<PayPalTransport["captureAuthorization"]>[0]): Promise<PayPalCaptureAuthorizationResponse> {
    ...
    const id = stringValue(body.id)
    if (!id || !stringValue(body.status)) throw new MedusaError(MedusaError.Types.INVALID_DATA, "PayPal capture response was malformed.")
  ...
  async retrieveOrder(input: Parameters<PayPalTransport["retrieveOrder"]>[0]): Promise<PayPalRetrieveOrderResponse> {
    ...
    const id = stringValue(body.id)
    if (!id || !stringValue(body.status)) throw new MedusaError(MedusaError.Types.INVALID_DATA, "PayPal retrieve-order response was malformed.")
    return { order_id: id, status: String(body.status), ...paymentEvidence(body) }
  }

  async retrieveCapture(input: { capture_id: string }): Promise<PayPalRetrieveCaptureResponse> {
    const body = await this.request("/v2/payments/captures/" + encodeURIComponent(input.capture_id), { method: "GET" })
    const id = stringValue(body.id)
    if (!id || !stringValue(body.status)) throw new MedusaError(MedusaError.Types.INVALID_DATA, "PayPal retrieve-capture response was malformed.")
    const relatedIds = body.supplementary_data && typeof body.supplementary_data === "object" ? (body.supplementary_data as Record<string, unknown>).related_ids : undefined
    const orderId = relatedIds && typeof relatedIds === "object" ? stringValue((relatedIds as Record<string, unknown>).order_id) : undefined
    return { capture_id: id, status: String(body.status), ...(orderId ? { order_id: orderId } : {}), ...money(body.amount) }
  }
  ...
  async refundCapture(input: Parameters<PayPalTransport["refundCapture"]>[0]): Promise<PayPalRefundCaptureResponse> {
    ...
    const id = stringValue(body.id)
    if (!id || !stringValue(body.status)) throw new MedusaError(MedusaError.Types.INVALID_DATA, "PayPal refund response was malformed.")
    return { refund_id: id, status: String(body.status), ...money(body.amount) }
  }

  async retrieveRefund(input: Parameters<PayPalTransport["retrieveRefund"]>[0]): Promise<PayPalRetrieveRefundResponse> {
    const body = await this.request("/v2/payments/refunds/" + encodeURIComponent(input.refund_id), { method: "GET" })
    const id = stringValue(body.id)
    if (!id || !stringValue(body.status)) throw new MedusaError(MedusaError.Types.INVALID_DATA, "PayPal refund read-back response was malformed.")
    return { refund_id: id, status: String(body.status), ...money(body.amount) }
  }

  async verifyWebhook(input: Parameters<PayPalTransport["verifyWebhook"]>[0]): Promise<boolean> {
    const payload = input.payload as { headers?: Record<string, string>; rawData?: string; data?: unknown }
    const headers = payload.headers || {}
    let webhookEvent = payload.data
    if (typeof payload.rawData === "string" && payload.rawData.length > 0) {
      try { webhookEvent = JSON.parse(payload.rawData) as unknown } catch { return false }
    }
    const body = await this.request("/v1/notifications/verify-webhook-signature", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ auth_algo: headers["paypal-auth-algo"], cert_url: headers["paypal-cert-url"], transmission_id: headers["paypal-transmission-id"], transmission_sig: headers["paypal-transmission-sig"], transmission_time: headers["paypal-transmission-time"], webhook_id: input.webhook_id, webhook_event: webhookEvent }) })
